...
resource "aws_route" "igw" {
  for_each               = local.no_igw ? [] : toset(var.availability_zones)
  route_table_id         = aws_route_table.public[each.key].id
...
resource "aws_eip" "nat_gw" {
  for_each = ( local.no_natgw ? [] : var.single_nat_gateway ? [var.availability_zones[0]] : toset(var.availability_zones) )
  domain = "vpc"
}
