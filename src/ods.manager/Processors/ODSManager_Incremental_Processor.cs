...
namespace Theradex.ODS.Manager.Processors
{
    public class ODSManager_Incremental_Processor : BaseProcessor, IProcessor
    {
        ...
        public async Task<bool> ProcessAsync(ManagerInput exInput)
        {
            ...
            {
                _logger.LogError($"TraceId:{_appSettings.TraceId}; - Error processing ${ex.ToString()}.");
                return false;
            }
        }
        ...
        private async Task GetODSTableIntervalIncrementalDate(ManagerInput exInput)
        {
            try
            {
                ...
                if (odsmanager_table_matadatas.Any())
                {
                    ...
                    {
                        //if (odsmanager_table_matadata.TableName.ToUpper() != "REPORTINGRECORDS")
                        //{
                        //    continue;
                        //}
                        ...
                        // Fetch the last successful run's end date
                        var lastSuccessfulRun = await _odsRepository.GetByMaxApiEndDateAsync(odsmanager_table_matadata.TableName);

                        if (lastSuccessfulRun != null && DateTime.TryParse(lastSuccessfulRun.ApiEndDate, out DateTime lastEndDate))
                        {
                            // Set the start date for the next interval to be one Hour before the last successful end date
                            minDate = lastEndDate.AddHours(-1);
                            _logger.LogInformation($"TraceId:{_appSettings.TraceId}; [Id: {odsmanager_table_matadata.Id}] [Table:{odsmanager_table_matadata.TableName}] - Using last successful end date: {minDate}");
                        }
                        else if (!string.IsNullOrEmpty(exInput.StartFrom) && DateTime.TryParse(exInput.StartFrom, out DateTime startFromDate))
                        {
                            minDate = startFromDate.AddHours(-2);
                            _logger.LogInformation($"TraceId:{_appSettings.TraceId}; [Id: {odsmanager_table_matadata.Id}] [Table:{odsmanager_table_matadata.TableName}] - Using provided StartFrom date: {minDate}");
                        }
                        else
                        {
                            _logger.LogInformation($"TraceId:{_appSettings.TraceId}; [Id: {odsmanager_table_matadata.Id}] [Table:{odsmanager_table_matadata.TableName}] - Using default min date from payload or 01/01/2000 if invalid.");
                        }

                        ODSData oDSData = new ODSData();
                        oDSData.TableName = odsmanager_table_matadata.TableName;
                        oDSData.StartDate = minDate;
                        oDSData.EndDate = DateTime.UtcNow;

                        //if (oDSData.StartDate >= DateTime.UtcNow)
                        //{
                        //    _logger.LogInformation($"TraceId:{_appSettings.TraceId}; [Id: {odsmanager_table_matadata.Id}] [Table:{odsmanager_table_matadata.TableName}] - Start date is in the future, skipping.");
                        //    continue;
                        //}
        ...
        {
            //if (batchRunControl.TableName.ToUpper() != "REPORTINGRECORDS")
            //{
            //    return null;
            //}
            ...
            try
            {
                HashSet<string> generatedIntervals = new HashSet<string>(); // To track generated intervals
                ...
                // Go back 1 hour from the intended start date
                DateTime startDate = odsData.StartDate.AddHours(-1);
                DateTime currentEndDate = DateTime.UtcNow;
                DateTime intervalEndDate = currentEndDate.AddMinutes(30);

                // Check if the difference between currentEndDate and startDate is more than 2 hours
                if (currentEndDate - startDate > TimeSpan.FromHours(12))
                {
                    // If it is, make intervalEndDate greater than startDate + 2 hours
                    intervalEndDate = startDate.AddHours(2).AddMinutes(1); // Adding 1 minute to ensure it's strictly greater
                }

                while (startDate <= currentEndDate)
                {
                    string formattedStartDate = startDate.ToString("yyyy-MM-ddTHH:mm:ss.fffffff");
                    string formattedEndDate = intervalEndDate.ToString("yyyy-MM-ddTHH:mm:ss.fffffff");
                    string bucket = $"{formattedStartDate}_{formattedEndDate}";

                    // Check if this interval has already been generated
                    if (!generatedIntervals.Contains(bucket))
                    {
                        try
                        {
                            ...
                            generatedIntervals.Add(bucket); // Mark this interval as generated
                        }
                        catch (Exception ex)
                        {
                    ...
                    {
                        _logger.LogInformation($"TraceId:{_appSettings.TraceId}; [Table:{batchRunControl.TableName}] Skipped duplicate interval:{bucket}");
                    }

                    // Move the start date to the beginning of the next interval
                    startDate = intervalEndDate.AddMilliseconds(-1);
                    intervalEndDate = startDate.AddHours(1);
                    intervalEndDate = intervalEndDate.AddMilliseconds(-1);
    ...
    public class ODSManager_Incremental_Processor2 : BaseProcessor, IProcessor
    {
        static List<string> ALLODSTABLES = new List<string>
                            {
                                "DATAPOINTROLESTATUS",
        ...
        public ODSManager_Incremental_Processor2(
            IMedidataRWSService medidateRWSService,
            ILogger<ODSManager_Processor> logger,
            IOptions<AppSettings> appOptions,
            IAWSCoreHelper awsCoreHelper,
